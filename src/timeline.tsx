...
export default function StatusItem({ status: originalStatus, revalidate }: Props) {
  ...
  return (
    <List.Item
      ...
      accessories={[isVisiblityPrivate(status.visibility) && { icon: getIconForVisibility(status.visibility) }].filter(
        Boolean
      )}
      detail={
        <List.Item.Detail
          markdown={dedent`
            ## ${status.account.displayName.replace(":verified_coffee:", "![](verified2.png)")} \`${
            status.account.acct
          }\`
